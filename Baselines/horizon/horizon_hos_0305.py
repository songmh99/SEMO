...
def export_res(pattern_expressions, dirty_path):
    ...
    res_path = "./Repaired_res/horizon/" + task_name[:-1] +"/repaired_" + task_name + file_name + ".csv"
        ...
        # for row in reader:
            ...
            for v in g.vertList:
                if g.vertList[v].type == 0:
                    ...
                    # print(f"Rtable {i} 111 : {Rtable}")
                    # print(f'Rtable:{Rtable}')
                    # return 0
            for j in range(0, len(OrderedFDs)):
                ...
                if PERFECTED:
                    if (i, list(clean_df.columns).index(OrderedFDs[j].right)) not in gt_wrong_cells:
                        ...
                        # print(f"Rtable {j} 444 : {Rtable}")
                    if (i, list(clean_df.columns).index(OrderedFDs[j].left)) not in gt_wrong_cells:
                        Rtable.update({OrderedFDs[j].left: data.loc[i, OrderedFDs[j].left]})
                        # print(f"Rtable {j} 555 : {Rtable}")
            # print(f"i is {i}, Rtabel 666: {Rtable}")
...
rule_path = "./data_with_rules/hospital/0310-test-rule.txt"
...
if True:
    if PERFECTED:
        out_path = "./Exp_result/horizon/" + task_name[:-1] +"/perfectED+EC_" + task_name + '_' + file_name + ".txt"
        res_path = "./Repaired_res/horizon/" + task_name[:-1] +"/perfect_repaired_" + task_name + '_' + file_name + ".csv"
        ...
        out_path = "./Exp_result/horizon/" + task_name[:-1] +"/onlyED_" + task_name + '_' + file_name + ".txt"
        if not os.path.exists(os.path.dirname(out_path)):
            os.makedirs(os.path.dirname(out_path))
        f = open(out_path, 'w')
        sys.stdout = f
        ...
        out_path = "./Exp_result/horizon/" + task_name[:-1] +"/oriED+EC_" + task_name + '_' + file_name + ".txt"
        f = open(out_path, 'w')
        sys.stdout = f
        res_path = "./Repaired_res/horizon/" + task_name[:-1] +"/repaired_" + task_name + '_' + file_name + ".csv"
        if not os.path.exists(os.path.dirname(res_path)):
            os.makedirs(os.path.dirname(res_path))
    print(rep_precision)
    print(rep_recall)
