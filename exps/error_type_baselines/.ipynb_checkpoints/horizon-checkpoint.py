        ...
        for line in reader:
            for v in dirty_dict[cnt]:
                if dirty_dict[cnt][v] != line[v]:
                    try:
                        print(str((cnt, v)) + "| dirty: " + dirty_dict[cnt][v] + "| clean:" + line[v] + "| repaired:" + pattern_expressions[cnt][str(v)])
                        if pattern_expressions[cnt][v] == line[v]:
                            correct += 1
                        else:
                            prd_w2w.append((cnt, v))
                    except:
                        print(str((cnt, v)) + "| dirty: " + dirty_dict[cnt][v] + "| clean:" + line[v] + "| repaired:  NULL" )
                        pass
                    tot += 1
                # if dirty_dict[cnt][v] != line[v]:
...
# dirty_path = 'data/hospital/hospital-inner_outer_error-20.csv'
# file_name = dirty_path.split('/')[-1].split('.')[0]
...
horizon_res_path = sys.argv[3]
print(f"horizon_res_path : {horizon_res_path}")
...
# horizon_res_path = 'Repaired_res/horizon_hos_0305.csv'
# horizon_res_path = config['horizon_res_path']
# horizon_res_path = sys.argv[2]
res_df.to_csv(horizon_res_path, index=False)