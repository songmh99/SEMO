...
# BERTtiny_Bert = BertModel.from_pretrained(bert_tiny_path).to(device)
# text_embed_dim = config['text_embed_dim']



###################################################
class CellDataset4(Dataset):
    ...
    def process_data(self):
        ...
        for i in tqdm(range(self.repair_res.shape[0]), desc="Processing Data", unit="sample"):
            ...
            error_value_embed = self.encode_text(error_value)
            label = row['onehot_label']
            col_content = ' '.join([str(v) for v in self.dirtydata.iloc[row_idx].values])
            row_embed = self.encode_text(col_content)
...
class cellclsmodel4(nn.Module):
    def __init__(self, embed_dim, hidden_dim, num_classes):
        super(cellclsmodel4, self).__init__()
...
from datetime import datetime
# 训练循环
def train4(model, dataloader, criterion, optimizer, epochs, device, res_file):
    model.train()
    f = open(res_file, 'a')
    # 获取当前时间
...
def test4(model, dataloader, device, res_file):
    model.eval()  # 设为评估模式
    correct = 0
    total = 0
    f = open(res_file, 'a')
    ...
    with torch.no_grad():  # 关闭梯度计算
        for batch in dataloader:
            ...
            # 获取预测结果
            outputs = model(col_name_embed, error_value_embed, row_embed, col_embed)  # (batch_size, num_classes)
...
def compute_mlp_metric4(model, dataloader,config, res_file, budget):
    ## 得到 mlp 预测结果
    model.eval()
    all_outputs = []
    with torch.no_grad():  # 关闭梯度计算
