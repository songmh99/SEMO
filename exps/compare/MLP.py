...
class cellclsmodel2(nn.Module):
    def __init__(self, embed_dim, hidden_dim, num_classes):
        super(cellclsmodel2, self).__init__()
...
from datetime import datetime
# 训练循环
def train2(model, dataloader, criterion, optimizer, epochs, device, res_file):
    model.train()
    f = open(res_file, 'a')
    # 获取当前时间
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            optimizer.zero_grad()
            outputs = model(col_name_embed, error_value_embed)
...
def test2(model, dataloader, device, res_file):
    model.eval()  # 设为评估模式
    correct = 0
    total = 0
    f = open(res_file, 'a')
    ...
    with torch.no_grad():  # 关闭梯度计算
        for batch in dataloader:
            ...
            # 获取预测结果
            outputs = model(col_name_embed, error_value_embed)  # (batch_size, num_classes)
...
def compute_mlp_metric2(model, dataloader,config, res_file,budget):
    ## 得到 mlp 预测结果
    model.eval()
    all_outputs = []
    with torch.no_grad():  # 关闭梯度计算
